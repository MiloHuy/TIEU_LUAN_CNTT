export default class DomHandler {
  ...
  static absolutePosition(element, target, align = "left") {
    if (element && target) {
      ...
      const alignOffset =
        align === "left" ? 0 : elementOuterWidth - targetOuterWidth;

      if (
        targetOffsetPx + targetOuterWidth + elementOuterWidth >
        viewport.width
      )
        left = Math.max(
          0,
  ...
  static flipfitCollision(
    ...
    if (element && target) {
      ...
      const getPositionValue = (arr, isOffset) =>
        isOffset
