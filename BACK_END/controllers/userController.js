    ...
    try {
        if (req.params.id == req.user._id) {
            return res.status(400).json({
                ...
                code: 4000,
                message:
                    "Phải dùng id của người khác, không được dùng id của bản thân.",
            });
        }
    ...
    try {
        ...
        if (!is_follow) {
            const [posts, totals] = await Promise.all([
                Post.find({ user_id: req.params.id, privacy: 2 })
                    .select("_id post_img.url")
                    .sort({ create_post_time: -1 })
                    .limit(Number(size))
                    .skip(skip)
                    .lean(),
                Post.countDocuments({ user_id: req.params.id, privacy: 2 }),
            ]);
        ...
        const [posts, totals] = await Promise.all([
            Post.find({ user_id: req.params.id, privacy: { $in: [1, 2] } })
                .select("_id post_img.url")
                .sort({ create_post_time: -1 })
                .limit(Number(size))
                .skip(skip)
...
exports.getAll = async (req, res, next) => {
    try {
        ...
        const userQuery = User.find({ role_id: { $ne: 0 } }).select(
            "avatar.url first_name last_name gmail id department phone_number is_active"
        );
...
exports.getAllUser = async (req, res) => {
    try {
        const { size } = req.query;
        const userQuery = User.find({ role_id: { $ne: 0 } }).select(
            "avatar.url first_name last_name department"
        );
